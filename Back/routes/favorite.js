...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        if (!user) {
            res.status(400).json({ message: '제공된 세션 ID에 해당하는 사용자를 찾을 수 없습니다.' });
        }
        // 사용자의 즐겨찾기에 이미 레스토랑이 있는지 확인합니다.
        const existingFavorite = await Favorites.findFavorite(user.id, restaurantId);
        if (existingFavorite) {
            res.status(400).json({ message: '이미 즐겨찾기에 추가된 식당입니다.' });
        }
        else {
            // 새로운 즐겨찾기 항목을 생성합니다.
        ...
        res.status(500).json({ message: '서버 오류로 인해 즐겨찾기를 추가할 수 없습니다.' });
    }
});
